...
<div class="flex flex-col sm:flex-row gap-4">
  ...
  />
  <figure class="w-lg mt-0">
    <Image 
      src={goodreadsUnstyledImg}
      alt="Goodreads collection data (unstyled)"
    />
...
<figure>
  ...
  <Image 
    src={goodreadsTailwindImg}
    alt="Goodreads collection data (styled using Tailwind)"
  />
  <figcaption class="text-sm text-center">
...
## Using plain JavaScript to retrieve Goodreads data

If you're still on Astro 4.x and below, you can't use the Content Loader API to retrieve data from Goodreads, but you can still use plain JavaScript to fetch the data and then pass it to your components as props. 

```astro title="src/components/mdx/GoodreadsGrid.astro" wrap=false del="fetch" del={3-16, 19} del={"I added a timeout & retry logic for fetching data to minimize any errors":8} mark={"Since I alreadyhad code for fetching data & parsing to a schema, it was easy to transition to using the Content Loader API": 21-32}
import xml2js from 'xml2js'
